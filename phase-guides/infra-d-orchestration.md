...
as the _last_ step means every piece it needs (containers, IaC, CI/CD) already
exists.
...
- **Launch types — EC2 vs Fargate:** with the **EC2 launch type**, ECS runs your
  containers on EC2 servers _you_ still own (you keep the mental model you know).
...
1. **ECS cluster (EC2 launch type) via Terraform.** Run your services on ECS while
   still on EC2. _Why:_ learn ECS's concepts (tasks, services) without also changing
...
2. **Load balancer + CloudWatch logging.** Put an ALB in front; ship logs/metrics to
   CloudWatch. _Why:_ a stable entry point and visibility into what your containers
   are doing.
3. **Switch launch type to Fargate.** _Why:_ drop server management entirely — no
